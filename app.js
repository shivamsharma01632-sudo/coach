...
function loadState() {
  ...
  } catch(e) { console.warn('FlowCoach: failed to load state', e); }
  // Ensure array properties are always valid arrays
  if (!Array.isArray(state.selectedRoutines)) state.selectedRoutines = ['fitness','nutrition','mindfulness','sleep'];
  if (!Array.isArray(state.completedToday)) state.completedToday = [];
  if (!Array.isArray(state.earnedAchievements)) state.earnedAchievements = [];
  if (!Array.isArray(state.customThemes)) state.customThemes = [];
  if (!Array.isArray(state.chatHistory)) state.chatHistory = [{ role:'system', content: SYSTEM_PROMPT }];
  // Filter out routines whose metadata is missing (e.g. custom routines lost on reload)
  state.selectedRoutines = state.selectedRoutines.filter(id => !!ROUTINE_META[id]);
  if (state.selectedRoutines.length === 0) state.selectedRoutines = ['fitness','nutrition','mindfulness','sleep'];
}
...
function animateXpBar() {
  ...
  fill.addEventListener('animationend', () => fill.classList.remove('level-up'), { once: true });
}
...
function initApp() {
  ...
  (state.selectedRoutines || []).slice(0,3).forEach(id => {
    ...
    pill.textContent = m.icon + ' ' + m.label.split(' ')[0];
...
function buildTodayRoutines() {
  ...
  state.selectedRoutines.forEach(id => {
    const m = ROUTINE_META[id];
    if (!m) return; // skip routines with missing metadata
    ...
    card.innerHTML = `
      <button class="rc-complete-btn${done ? ' checked' : ''}" title="${done ? 'Completed' : 'Mark as complete'}" onclick="event.stopPropagation(); if(!${done}) markRoutineDone('${id}')">
        ${done ? '✓' : ''}
      </button>
...
let lastPoseAngle = 0;
let poseTrail = { leftWrist: [], rightWrist: [] };

const EXERCISE_CONFIGS = {
  squat:  { name:'Squats',     upAngle:160, downAngle:110, joint:['leftHip','leftKnee','leftAnkle'],
            icon:'🏋️', startPos:'Stand shoulder-width apart, toes slightly outward. Keep your back straight and arms forward. Make sure your full body is visible.',
            guideStep:'Set up for Squats', guideTip:'Stand 2–3 m away so your full body fits in frame. Feet shoulder-width apart.' },
  pushup: { name:'Push-ups',   upAngle:160, downAngle:90,  joint:['leftShoulder','leftElbow','leftWrist'],
            icon:'💪', startPos:'Get into a plank position facing the camera side-on. Body flat, hands under shoulders. Camera should see your full side profile.',
            guideStep:'Set up for Push-ups', guideTip:'Position your camera at floor level to your side so it can see your full body in plank.' },
  curl:   { name:'Bicep Curl', upAngle:160, downAngle:60,  joint:['leftShoulder','leftElbow','leftWrist'],
            icon:'🦾', startPos:'Stand facing the camera, arms at your sides holding weights. Keep elbows tucked in and your full upper body visible.',
            guideStep:'Set up for Bicep Curls', guideTip:'Face the camera directly. Step back until your arms and upper body fully fit in frame.' },
  lunge:  { name:'Lunges',     upAngle:170, downAngle:100, joint:['leftHip','leftKnee','leftAnkle'],
            icon:'🤸', startPos:'Stand upright facing the camera side-on. Feet together. Step forward to lunge — camera should see your full side profile.',
            guideStep:'Set up for Lunges', guideTip:'Position camera to your side so it can see your full body when you step forward.' },
};

/* ── GUIDE OVERLAY STATE ── */
let guidePhase   = 'idle'; // idle | setup | countdown | tracking
let countdownVal = 3;
let countdownTimer = null;
let checksPassed = { distance: false, light: false, space: false };

async function startCamera() {
  const overlay   = document.getElementById('camOverlay');
  const guideOver = document.getElementById('bodyGuideOverlay');
  const startBtn  = document.getElementById('startCamBtn');
  const stopBtn   = document.getElementById('stopCamBtn');

  // Show loading state
  overlay.innerHTML = `
    <div class="cam-overlay-icon" style="animation:float 1.5s ease-in-out infinite">⏳</div>
    <div class="cam-overlay-text">Starting camera &amp; loading AI engine…</div>`;
  ...
  try {
    ...
    // Load PoseNet in parallel
    if (!poseNet) {
      overlay.innerHTML = `
        <div class="cam-overlay-icon" style="animation:float 1.5s ease-in-out infinite">🧠</div>
        <div class="cam-overlay-text">Loading AI pose engine… (first load may take ~10s)</div>`;
    ...
    // Hide loading overlay and begin tracking immediately
    overlay.classList.add('hidden');
    guideOver.style.display = 'none';
    ...
    overlay.innerHTML = `
      ...
      <div class="cam-overlay-text">${
        e.name === 'NotAllowedError'
          ? 'Camera permission denied.<br>Please allow camera access and try again.'
          : 'Could not start camera:<br>' + e.message
      }</div>`;
  }
}

function showGuideSetup() {
  guidePhase = 'setup';
  checksPassed = { distance:false, light:false, space:false };

  const cfg = EXERCISE_CONFIGS[currentExercise];

  // Update guide text
  document.getElementById('guideStep').textContent = cfg.guideStep;
  document.getElementById('guideTip').textContent  = cfg.guideTip;

  // Reset checklist
  ['distance','light','space'].forEach(k => {
    const el = document.getElementById('chk-'+k);
    if (el) { el.classList.remove('done'); el.querySelector('.chk-icon').textContent = '○'; }
  });

  document.getElementById('guideChecklist').style.display = 'flex';
  document.getElementById('guideCountdown').style.display = 'none';

  // Add exercise badge on top of camera
  let badge = document.getElementById('poseGuideBadge');
  if (!badge) {
    badge = document.createElement('div');
    badge.className = 'pose-guide-badge';
    badge.id = 'poseGuideBadge';
    document.querySelector('.cam-wrap').appendChild(badge);
  }
  badge.textContent = cfg.icon + ' ' + cfg.name + ' — Get Ready';
  badge.style.display = 'block';

  // Silhouette colour = waiting (accent)
  document.getElementById('silhouetteSvg').classList.remove('ready');

  // Run a lightweight pose check loop to tick off checklist items
  runGuideChecks();
}

let guideCheckId = null;
function runGuideChecks() {
  if (guideCheckId) cancelAnimationFrame(guideCheckId);

  async function checkFrame() {
    if (guidePhase !== 'setup') return;
    const video = document.getElementById('camVideo');
    if (!video.videoWidth) { guideCheckId = requestAnimationFrame(checkFrame); return; }

    try {
      const pose = await poseNet.estimateSinglePose(video, { flipHorizontal:true });

      // Check 1: body visible for selected exercise
      const needed = currentExercise === 'curl'
      ...
      const allVisible = needed.every(p => {
        const kp = pose.keypoints.find(k => k.part === p);
        return kp && kp.score > 0.35;
      });
      tickCheck('distance', allVisible);

      // Check 2: lighting (overall pose confidence)
      tickCheck('light', pose.score > 0.4);

      // Check 3: space / background (pose score on extremities)
      const ankleL = pose.keypoints.find(k => k.part === 'leftAnkle');
      const ankleR = pose.keypoints.find(k => k.part === 'rightAnkle');
      const hasAnkles = (ankleL && ankleL.score > 0.25) || (ankleR && ankleR.score > 0.25);
      tickCheck('space', hasAnkles);

      // If all 3 passed, start countdown
      if (checksPassed.distance && checksPassed.light && checksPassed.space) {
        document.getElementById('silhouetteSvg').classList.add('ready');
        document.getElementById('guideStep').textContent = "Perfect! Hold still…";
        setTimeout(startCountdown, 600);
        return;
      }
    } catch(e) {}

    guideCheckId = requestAnimationFrame(checkFrame);
  }
  checkFrame();
}

function tickCheck(key, passed) {
  if (checksPassed[key] === passed) return;
  checksPassed[key] = passed;
...
function startCountdown() {
  if (guidePhase !== 'setup') return;
  guidePhase = 'countdown';
  countdownVal = 3;

  document.getElementById('guideChecklist').style.display = 'none';
  document.getElementById('guideCountdown').style.display = 'flex';
  document.getElementById('countdownNum').textContent = countdownVal;

  const badge = document.getElementById('poseGuideBadge');
  if (badge) badge.textContent = '🚀 Starting in…';
...
function beginTracking() {
  ...
  // Hide guide overlay
  const guideOver = document.getElementById('bodyGuideOverlay');
  guideOver.style.opacity = '0';
  guideOver.style.transition = 'opacity 0.5s ease';
  setTimeout(() => { guideOver.style.display = 'none'; guideOver.style.opacity = '1'; }, 500);

  // Remove badge
  const badge = document.getElementById('poseGuideBadge');
  if (badge) badge.style.display = 'none';

  // Show pose status bar
  document.getElementById('poseStatus').style.display = 'flex';
  ...
  updateCamStats();
  trackPose();
  showToast('🏋️ Tracking started — Go!');
  addXP(5, 'Started workout');
}
...
function drawPose(ctx, canvas, pose) {
  ...
  // Record the last positions for wrists so the motion path is visible.
  ['leftWrist','rightWrist'].forEach(part => {
    const kp = kps[part];
    if (kp && kp.score > 0.35) {
      poseTrail[part].push({ x: kp.position.x, y: kp.position.y });
      if (poseTrail[part].length > 20) poseTrail[part].shift();
    }
  });

  // Draw wrist motion trails
  ctx.lineWidth = 4;
  ctx.lineCap = 'round';
  ['leftWrist','rightWrist'].forEach((part, index) => {
    const trail = poseTrail[part];
    if (trail.length < 2) return;
    ctx.beginPath();
    trail.forEach((pt, idx) => {
      const alpha = (idx + 1) / trail.length * 0.6;
  ...
  // Highlight the primary rep joint for the selected exercise
  const cfg = EXERCISE_CONFIGS[currentExercise];
  const primary = cfg.joint.map(part => kps[part]).filter(kp => kp && kp.score > 0.35);
  primary.forEach(kp => {
    ctx.beginPath();
...
function refreshExerciseTip() {
  ...
  if (title) title.textContent = `${cfg.name} — Starting Position`;
  if (text) text.textContent = cfg.startPos;
}
...
function applyTheme(id, silent = false) {
  ...
  let th = THEMES.find(t => t.id === id);
  if (!th && state.customThemes) {
    th = state.customThemes.find(t => t.id === id);
  }
  if (!th) th = THEMES[0];
  
  if (th.isCustom) {
    document.documentElement.style.setProperty('--hue', th.hue);
  ...
  document.querySelectorAll('.t-swatch').forEach(sw => {
    sw.classList.toggle('active', sw.title === th.label);
  });
  
  saveState();
  if (!silent) showToast(`Theme ${th.label} applied ✨`);
}

function toggleCustomThemeForm() {
  const form = document.getElementById('customThemeForm');
  const btn = document.getElementById('toggleThemeFormBtn');
  if (!form || !btn) return;
  
  if (form.style.display === 'none' || !form.style.display) {
    form.style.display = 'flex';
    btn.innerHTML = 'Cancel Custom Theme';
  } else {
    form.style.display = 'none';
    btn.innerHTML = '🎨 Create Custom Theme';
  }
}

function updateHuePreview(val) {
  const label = document.getElementById('hueSliderLabel');
  const preview = document.getElementById('huePreviewBlock');
  if (label) label.textContent = `Accent Color (Hue: ${val}°)`;
  if (preview) preview.style.background = `hsl(${val}, 72%, 52%)`;
}
...
function addCustomTheme() {
  ...
  const hue = document.getElementById('customThemeHue').value;
  const bg = document.getElementById('customThemeBg').value;
  const bg2 = document.getElementById('customThemeBg2').value;
  
  const accent1 = `hsl(${hue},72%,52%)`;
  const accent2 = `hsl(${(Number(hue)+40)%360},70%,60%)`;
  const gradient = `linear-gradient(135deg, ${accent1}, ${accent2})`;
...
function removeCustomTheme(id) {
  ...
  saveState();
  showToast('Custom theme removed 🗑️');
}
...
function confirmReset() {
  ...
  state.userName = '';
  state.selectedChar = 'seed';
  state.currentTheme = 'default';
  state.selectedRoutines = ['fitness','nutrition','mindfulness','sleep'];
  state.scheduleData = {};
  state.completedToday = [];
  state.xp = 0;
  state.level = 1;
  ...
  state.routinesCompletedTypes = 0;
  state.earnedAchievements = [];
  state.chatHistory = [{ role:'system', content: SYSTEM_PROMPT }];
  
  // Reset theme class and swatch active state (Forest)
  document.documentElement.dataset.theme = '';
  document.querySelectorAll('.t-swatch').forEach(sw => {
    sw.classList.toggle('active', sw.title === 'Forest');
  });

  // Reset name input and previews
  const nameInput = document.getElementById('nameInput');
  if (nameInput) nameInput.value = '';
  const namePreview = document.getElementById('namePreview');
  if (namePreview) namePreview.textContent = 'friend';
  
  // Reset onboarding routine chip selection
  document.querySelectorAll('.routine-chip').forEach(chip => {
    chip.classList.toggle('on', state.selectedRoutines.includes(chip.dataset.id));
  });

  closeResetModal();
  
  // Transition screens back to welcome
  document.getElementById('appScreen').classList.add('hidden');
  document.getElementById('welcomeScreen').classList.remove('hidden');
  
  showToast('Profile reset! Create a new one. 🌱');
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try { applyTheme(state.currentTheme || 'default', true); } catch(e) { console.warn('Theme error:', e); }

  // Build onboard theme swatches
  try { buildThemeSwatches('onboardThemePalette'); } catch(e) {}

  // Mark pre-selected routine chips
  document.querySelectorAll('.routine-chip').forEach(chip => {
    if (state.selectedRoutines && state.selectedRoutines.includes(chip.dataset.id)) chip.classList.add('on');
  });

  // Initialize exercise tip card
  try { refreshExerciseTip(); } catch(e) {}
  ...
  if (state.userName) {
    ...
    try {
      ...
      console.error('FlowCoach launch error:', e);
    }
    // ALWAYS ensure appScreen is visible for returning users
